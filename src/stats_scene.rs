...
pub fn render(state: &State) {
    ...
    for (text, i) in COLUMNS.iter().zip(0..) {
        ...
        draw_text(text, &state.font, point, theme.primary);
    }
    ...
    if let Some(stats) = &app.stats {
        for (n, i) in stats.launches.iter().zip(0..) {
            render_info(&state.font, color, 2, i, &format!("{n}"));
        }
        let installed_on = format_date(stats.installed_on);
        render_info(&state.font, color, 3, 0, &installed_on);
        let updated_on = format_date(stats.updated_on);
        render_info(&state.font, color, 4, 0, &updated_on);
    }
    if let Some(button_group) = &state.button_group {
        button_group.render(&state.font, &state.settings.theme);
    }
}

fn render_info(font: &FontBuf, color: Color, i: i32, j: i32, t: &str) {
